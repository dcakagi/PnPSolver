...
if __name__ == '__main__':
    ...
    dx = .066617 * (50/(np.arange(N)-10075)+.7)
    ...
    N = 10
    zs = np.arange(zs[-1],end_wrt_start[2],1)
    ...
    # for w in waypoints:
        ...
        if not flag:
            rot = airsim.to_quaternion(np.deg2rad(pitch),0,0)
        else:
        ...
        if abs(pose.position.x_val) == 0 and only_once:
            pitch = 0
            switch = True
            only_once = False
            cam = "3"
            light_detector.current_colors = light_detector.descent_colors
            px_locations, position = process_im(cam)
            ...
            initial_estimator.setFocalLengthPixelsFromFOV(np.deg2rad(120))
            ...
            estimator_cv.setFocalLengthPixelsFromFOV(np.deg2rad(120))
            # continue
        else:
            px_locations, position = process_im(cam)
        print(len(px_locations.values()))
        if len(px_locations.values()) < 4:
            break
        ...
        distances.append(np.linalg.norm([x_val/100,y_val/100,z_val/100]))
        if switch:
            switch = False
    ...
    axs[1].annotate('switch to 120' + u"\N{DEGREE SIGN}" + ' FOV\nnadir camera', xy=(the_switch_x,the_switch_y1),  xycoords='data',
            xytext=(the_switch_x+100,the_switch_y1+0.3), textcoords='data',
            arrowprops=dict(facecolor='black', shrink=0.01,width=1.5,headwidth=6),
    ...
    axs[0].annotate('switch to 120' + u"\N{DEGREE SIGN}" + ' FOV\nnadir camera', xy=(the_switch_x,the_switch_y2),  xycoords='data',
            xytext=(the_switch_x+100,the_switch_y2+.3), textcoords='data',
            arrowprops=dict(facecolor='black', shrink=0.01,width=1.5,headwidth=6),
            horizontalalignment='center', verticalalignment='top',
            )
    plt.savefig("450m_Mission_Simulated.png", dpi=500)
    plt.show()  